...
\begin{enumerate}
    \item
        The chain \( X \) is \defn{reversible} if \( X \) is an
...
\). This is called \defn{Kolmogorov's loop criterion}.%
...
\begin{algorithm}[H]
  \KwData{Probability transition matrix \( P \)}
  ...
  \Repeat{There are no more states left to add}{
    ...
    \) and \( P_{i_2,i_1} \).\;
    \( S_1 \leftarrow \set{i_1, i_2} \).\;
    \If{ \(P_{i_1,i_2} = P_{i_2,i_1} \)}{move to the next step.}
    \Else{assume \( P_{i_1,i_2 }< P_{i_2,i_1} \).}
    Multiply row \( i_2 \) by \( P_{i_1,i_2}/P_{i_2,i_1} \)\;
    Adjust \( P_{i_2,i_2} \) to make the row sum to \( 1 \).\;
    \If{ \( P_{i_1,i_2} > P_{i_2,i_1} \)}
        Multiply column \( i_2 \) by \( P_{i_2,i_1}/P_{i_1,i_2} \);
        Adjust all diagonal entries so the rows sum to \( 1 \).\;
        \tcp*[h]{The new
        matrix \( P^{\star} \) will now have \( P_{i_1,i_2} = P_{i_2,i_1}^
        {\star} \).}
      
        Choose another state \( i_3 \) which has nonzero transition
        probabilities with a state in \( S_1 \).\;
        Make the appropriate
        row or column multiplication operation on row or column \( i_3 \)\;
        \( S_2 \leftarrow \set{i_1, i_2, i_3} \).}
         \tcp*[h]{After \( n - 1 \) steps we have \( S_n = \set{i_1,
           \dots, i_n}.}
